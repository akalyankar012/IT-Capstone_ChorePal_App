...
exports.geminiParseStrictJSON = geminiParseStrictJSON;
 ...
 */
async function geminiParseStrictJSON(system, user) {
    ...
    const out = data?.candidates?.[0]?.content?.parts?.[0]?.text ??
        ...
        "";
    if (!out)
        throw new Error("AI Studio returned empty response");
    return out;
}
/**
 * Parse transcript using Gemini to extract task fields
 */
async function parseTranscript(request) {
    try {
        const currentDate = request.currentDate || (0, time_1.getCurrentDateISO)();
...
Extract the task information. If anything is missing or unclear, ask ONE specific question.`;
        ...
        const validatedResponse = schema_1.ParseResultSchema.parse(parsedResponse);
        // If we have a complete result, validate the task fields
        if (!validatedResponse.needsFollowup && validatedResponse.result) {
            const taskFields = schema_1.TaskFieldsSchema.parse(validatedResponse.result);
