...
exports.findBestChildMatch = findBestChildMatch;
exports.generateChildFollowUpQuestion = generateChildFollowUpQuestion;
/**
 * Simple fuzzy matching for child names
 */
function findBestChildMatch(inputName, children) {
    if (children.length === 0) {
        return { match: null, isAmbiguous: false, candidates: [] };
    }
    const input = inputName.toLowerCase().trim();
    // Exact match first
    const exactMatch = children.find(child => child.name.toLowerCase() === input);
    if (exactMatch) {
        return { match: exactMatch, isAmbiguous: false, candidates: [exactMatch] };
    }
    // Starts with match
    const startsWithMatches = children.filter(child => child.name.toLowerCase().startsWith(input));
    if (startsWithMatches.length === 1) {
        return { match: startsWithMatches[0], isAmbiguous: false, candidates: startsWithMatches };
    }
    // Contains match
    const containsMatches = children.filter(child => child.name.toLowerCase().includes(input));
 ...
 */
function generateChildFollowUpQuestion(candidates) {
